    ...
    void render(const hittable& world, std::vector<uint32_t>& pixels) {
        ...
        for (int j = 0; j < image_height; j++) {
            ...
            for (int i = 0; i < image_width; i++) {
                ...
                uint32_t pixel = write_color(std::cout, pixel_samples_scale * pixel_color);
...
private:
    ...
    point3 pixel00_loc;         // Location of pixel 0, 0, the upper left pixel
    ...
    color ray_color(const ray& r, int depth, const hittable& world) const {
        ...
        if (world.hit(r, interval(0.001, infinity), rec)) {
            //vec3 direction = random_on_hemisphere(rec.normal); // random scattering in all directions
